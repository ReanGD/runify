...
type dataProviderHandler interface {
	getName() string
	init(cfg *config.Config, moduleLogger *zap.Logger, providerID uint64) error
	start()
	getRoot() []*pb.Command
}
...
func (p *dataProvider) init(cfg *config.Config, rootProviderLogger *zap.Logger) <-chan error {
	ch := make(chan error)
	go func() {
		...
		ch <- p.handler.init(cfg, p.ModuleLogger, p.providerID)
	}()

	return ch
}

func (p *dataProvider) start(ctx context.Context, wg *sync.WaitGroup) <-chan error {
	wg.Add(1)
	ch := make(chan error)
	go func() {
		p.handler.start()
